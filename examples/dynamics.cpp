 ...
 */
bool eventDetection(Model<double, 4>& model, Eigen::Matrix<double, 4, 1>& q, Eigen::Matrix<double, 4, 1>& p) {
    // Get the position of the left foot
    Eigen::Matrix<double, 3, 1> right_foot_pos = translation(model, q, world_frame, swing_foot);
    ...
    Eigen::Matrix<double, 3, 1> swing_from_planted = translation(model, q, planted_foot, swing_foot);
...
Eigen::Matrix<double, 8, 1> impact_mapping(Model<double, 4>& model,
    ...
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> Jap = null(Ja);
    // Compute the null space of Jb
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> Jbp = null(Jb);
...
int main(int argc, char* argv[]) {
    ...
    auto Ja                             = geometric_jacobian(model, q_minus, std::string("left_foot"));
    Eigen::Matrix<double, 3, 4> Ja_v    = Ja.block(0, 0, 3, 4);
    auto Jb                             = geometric_jacobian(model, q_minus, std::string("right_foot"));
