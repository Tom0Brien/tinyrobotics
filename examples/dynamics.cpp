 ...
 */
bool eventDetection(tr::Model<double, 4>& model, Eigen::Matrix<double, 4, 1>& q, Eigen::Matrix<double, 4, 1>& p) {
    // Get the position of the left foot
    Eigen::Matrix<double, 3, 1> right_foot_pos = tr::translation(model, q, world_frame, swing_foot);
    ...
    Eigen::Matrix<double, 3, 1> swing_from_planted = tr::translation(model, q, planted_foot, swing_foot);
...
Eigen::Matrix<double, 8, 1> impact_mapping(tr::Model<double, 4>& model,
    ...
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> Jap = tr::null(Ja);
...
int main(int argc, char* argv[]) {
    ...
    auto model               = tr::model_from_urdf<double, 4>(path_to_urdf);
    ...
    auto results                   = tr::solver(model, q0, p0, u0, params);
    ...
    auto Ja                             = tr::geometric_jacobian(model, q_minus, std::string("left_foot"));
    Eigen::Matrix<double, 3, 4> Ja_v    = Ja.block(0, 0, 3, 4);
    auto Jb                             = tr::geometric_jacobian(model, q_minus, std::string("right_foot"));
    ...
    auto results2 = tr::solver(model, q0, p0, u0, params);
