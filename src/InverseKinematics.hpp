...
namespace RML {
    ...
    public:
         ...
         */
        IKVariables(const std::string& name,
                    std::shared_ptr<Model<Scalar>> _model,
    ...
    public:
        ...
        IKCost(const std::string& name,
               ...
               const Eigen::Matrix<Scalar, Eigen::Dynamic, 1>& _q0)
            : CostTerm(name) {
            model            = _model;
         ...
         */
        static inline Eigen::Matrix<AutoDiffType, Eigen::Dynamic, Eigen::Dynamic> cost(
            const Eigen::Matrix<AutoDiffType, Eigen::Dynamic, 1>& q,
            ...
            Eigen::Matrix<AutoDiffType, Eigen::Dynamic, Eigen::Dynamic> W =
                Eigen::Matrix<AutoDiffType, Eigen::Dynamic, Eigen::Dynamic>::Identity(model->n_q, model->n_q);
            Eigen::Matrix<AutoDiffType, 3, 3> K = Eigen::Matrix<AutoDiffType, 3, 3>::Identity();
            Eigen::Matrix<AutoDiffType, Eigen::Dynamic, Eigen::Dynamic> cost =
        ...
        Scalar GetCost() const override {
            ...
            Eigen::Matrix<AutoDiffType, Eigen::Dynamic, 1> q_auto(
                GetVariables()->GetComponent("configuration_vector")->GetValues());

            // Evaluate the cost function
            Eigen::Matrix<AutoDiffType, Eigen::Dynamic, Eigen::Dynamic> cost_val =
    ...
    Eigen::Matrix<Scalar, Eigen::Dynamic, 1> inverse_kinematics(
        ...
        nlp.AddCostSet(std::make_shared<IKCost<double, autodiff::dual>>("IK_cost",
                                                                        autodiff_model,
                                                                        source_link_name,
                                                                        target_link_name,
                                                                        desired_pose,
