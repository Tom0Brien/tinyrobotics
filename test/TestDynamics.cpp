...
//         0.0072, 0.0402, 0.0072, -0.0186, 0.0008, 0, 0, -0.0186, -1.9636, -0.0186, 0.9620, -0.0130, -0.0730, 0,
//         0.0008, 0.0290, 0.0008, -0.0130, 0.0008, 0, 0, 0, 0.1354, 0, -0.0730, 0, 0.0122, 0, 0, 0, 0, 0, 0, 0, 0;
...
TEST_CASE("Test Forward Dynamics via Articulated-Body Algorithm for 3 link model", "[Dynamics]") {
    ...
    auto robot_model   = RML::model_from_urdf<double, n_joints>("data/urdfs/3_link.urdf");
    // Create some inputs
    Eigen::Matrix<double, n_joints, 1> q;
    q << 1, 2, 3;
    Eigen::Matrix<double, n_joints, 1> qd;
...
TEST_CASE("Test Forward Dynamics via Articulated-Body Algorithm for 4 link model", "[Dynamics]") {
    ...
    auto robot_model   = RML::model_from_urdf<double, n_joints>("data/urdfs/4_link.urdf");
    // Create some inputs
    Eigen::Matrix<double, n_joints, 1> q;
    q << 1, 2, 3, 4;
    Eigen::Matrix<double, n_joints, 1> qd;
...
TEST_CASE("Test Forward Dynamics via Articulated-Body Algorithm for 5 link model", "[Dynamics]") {
    ...
    auto robot_model   = RML::model_from_urdf<double, n_joints>("data/urdfs/5_link.urdf");
    // Create some inputs
    Eigen::Matrix<double, n_joints, 1> q;
    q << 1, 2, 3, 4, 5;
    Eigen::Matrix<double, n_joints, 1> qd;
...
TEST_CASE("Test Forward Dynamics via Articulated-Body Algorithm for panda_arm", "[Dynamics]") {
    ...
    auto robot_model   = RML::model_from_urdf<double, n_joints>("data/urdfs/panda_arm.urdf");
    // Create some inputs
    Eigen::Matrix<double, n_joints, 1> q;
    q << 1, 2, 3, 4, 5, 6, 7;
    Eigen::Matrix<double, n_joints, 1> qd;
